...
// Chave API do ScraperAPI (verificada)
...
async function scrapeShopeeProduct(url) {
    ...
    try {
        // Usando os parâmetros exatos conforme documentação oficial
        const encodedUrl = encodeURIComponent(url);
        const apiUrl = `http://api.scraperapi.com?api_key=${API_KEY}&url=${encodedUrl}&render=true`;
        
        console.log(`[ScraperAPI] Chamando API com formato recomendado oficial...`);
        ...
        console.error(`[ScraperAPI] ERRO durante a raspagem: ${error.message}`);
        
        // Informação detalhada para depuração
        if (error.response) {
            console.error(`[ScraperAPI] Status: ${error.response.status}`);
            console.error(`[ScraperAPI] Headers: ${JSON.stringify(error.response.headers)}`);
            console.error(`[ScraperAPI] Data: ${JSON.stringify(error.response.data)}`);
        }
